...
class DynamicDocumentEmbeddingConfig(PretrainedConfig):
    ...
    def __init__(self, model_name='./scibert-weights', **kwargs): # training starts from the biobert-checkpoint
...
class DynamicDocumentEmbeddingModel(PreTrainedModel):
    ...
    def get_document_embedding(self, query_input_ids, query_attention_mask, doc_input_ids, doc_attention_mask):
        with torch.no_grad():
            ...
            num_tokens = doc_attention_mask.sum(
                dim=1).float()      # [batch_size]
            num_tokens = num_tokens.unsqueeze(0).expand(
                batch_size, -1)  # [batch_size, batch_size]
            # print(attention_scores)
            attention_scores_no_inf = torch.where(attention_scores == float(
                '-inf'), torch.tensor(0.0), attention_scores)
            relevance_scores = attention_scores_no_inf.sum(
                dim=2) / num_tokens  # [batch_size, batch_size]

            # attention_weights = self.attention_layer.softmax(attention_scores, dim=-1) 
            attention_weights = self.attention_layer.softmax(
                attention_scores)
            # Compute document embeddings
            V = self.attention_layer.value_proj(doc_token_embeddings)  # [batch_size, seq_len - 1, hidden_size]
            V_expanded = V.unsqueeze(0).expand(batch_size, -1, -1, -1)  # [batch_size, batch_size, seq_len -1, hidden_size]
            ...
            return doc_embeddings, attention_weights, relevance_scores  # [batch_size, hidden_size], [batch_size, seq_len - 1]
